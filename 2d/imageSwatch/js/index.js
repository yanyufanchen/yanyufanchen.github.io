	...
	Frame = function () {
		...
		_createClass(Frame, [{
			...
			value: function update() {
				...
				this.translation[2] = Math.floor(this.translation[2])
				this.rotation[0] = Math.floor(this.rotation[0])
				this.rotation[1] = Math.floor(this.rotation[1])
				if (this.translation[2] % 2 == 0) {

				} else {
					// 奇数
					this.translation[2] = this.translation[2] + 1
	...
	Gallery = function () {
		...
		_createClass(Gallery, [{
			...
			value: function initFrames() {
				var _this2 = this;
				this.frames = [];
				$$('.frame').forEach(function (node) {
					return _this2.frames.push(new Frame(node));
				});
			}
		}, {
			key: 'resize',
